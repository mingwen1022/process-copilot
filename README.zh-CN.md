...
**产品详情、六个 Agent 的逐个拆解与完整截图**：<https://mingwen.net/projects/process-copilot.html>

| 设计 Agent：多源材料 → 标准定义 | 运维：高风险动作转授权签字 | 效能分析：算指标 + 归因 + 反哺 |
| --- | --- | --- |
| ![设计](docs/screenshots/design-agent.jpg) | ![授权](docs/screenshots/ops-authorization.jpg) | ![效能](docs/screenshots/perf-board.jpg) |

> 评测数字、设计取舍与当前边界都写在详情页，本 README 不重复——**避免两处数字打架**。这里只讲怎么跑起来、代码长什么样。
...
| **流程待办与在办副驾** | 审批人 / 发起人 | 待办排序与优先级、合规体检、解释单据当前状态；确定性动作走确认门 |
...
**闭环已跑通**：某流程经"运行诊断 → 反哺 → 重新设计(v2)"后，在合成事件日志上重算，平均办结时长 76h → 23h、违规跳级 7 → 0。
> ⚠️ 这是 **what-if 流程仿真**：管线和指标计算是真的，结构性改进（如补兜底路径后跳级归零）机械成立；但**改进幅度来自建模假设，不是已验证的业务 ROI**——运行态是演示 mock，事件日志为合成数据。
