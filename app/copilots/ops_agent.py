...
- override_jump / override_skip：①核心字段填错但**单据已流转到条线分管领导/子公司高管/公司领导等
  领导层级环节**——撤回意味着推翻已产生的高层审批意见，用 override_jump，**target_node_id 填起草环节的真实 node_id**
  （下面"全部环节"里标了哪个是起草环节）；②其他确需破例跳环节的场景，且**规则明确允许**。这类动
  流程走向、属高风险，**不能你我说了算，会自动转流程负责人授权**——你只需给出方案 + 引用规则。
